...
| padList | 输入 | padding列表 [padding_left, padding_right, padding_top,padding_bottom]，每个元素取值范围：[0,255]。默认为{0, 0,<br>`0, 0}。`<br>fmatrix 输入用于控制LoadData指令从left还是right寄存器获取信息。Mode FmatrixMode类型，定义如下。当前只支持FMATRIX_LEFT，左右矩阵均使用该配置。<br>`enum class FmatrixMode : uint8_t {`<br>`FMATRIX_LEFT = 0,`<br>`FMATRIX_RIGHT = 1,`<br>`};` |

### 约束说明

- 该接口需要配合load3Dv1/load3Dv2接口一起使用，需要在load3Dv1/load3Dv2
接口之前调用。
