...
l1H 输入源操作数height，取值范围：l1H∈[1, 32767]。

l1W 输入源操作数width，取值范围：l1W∈[1, 32767]。

padList 输入 padding列表 [padding_left, padding_right, padding_top,padding_bottom]，每个元素取值范围：[0,255]。默认为{0, 0,

```cpp
0, 0}。
```

fmatrix 输入用于控制LoadData指令从left还是right寄存器获取信息。Mode FmatrixMode类型，定义如下。当前只支持FMATRIX_LEFT，左右矩阵均使用该配置。
...
public:__aicore__ inline KernelLoad3d()
...
boundar 输入边界值。yValue Load3Dv1指令：单位是32字节。
