...
| fmatrix | 输入 | 用于控制LoadData指令从left还是right寄存器获取信息。Mode FmatrixMode类型，定义如下。当前只支持FMATRIX_LEFT，左右矩阵均使用该配置。<br>`enum class FmatrixMode : uint8_t {`<br>`FMATRIX_LEFT = 0,`<br>`FMATRIX_RIGHT = 1,`<br>`};` |
