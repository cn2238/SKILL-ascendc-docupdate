...
设置稀疏矩阵稠密化过程生成的索引矩阵。索引矩阵在稠密化中的作用请参考MmadWithSparse。
...
| 参数名 | 输入/输出 | 描述 |
| --- | --- | --- |
| indexGlob | 输入 | 索引矩阵在Global Memory上的首地址，类型为al GlobalTensor。索引矩阵的数据类型为int2，需要由用户拼成int8的数据类型，再传入本接口。索引矩阵的Format格式只支持NZ格式。 |
