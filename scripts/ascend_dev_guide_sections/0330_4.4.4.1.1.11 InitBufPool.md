...
关于TBufPool的具体介绍及资源划分图示请参考4.4.4.1.3 TBufPool。
...
len 输入新划分资源池长度，单位为Byte，非32Bytes对齐会自动补齐至32Bytes对齐。

shareBu 输入被复用资源池，类型为TBufPool，新划分资源池与被复用资源f 池共享起始地址及长度。

### 约束说明

- 新划分的资源池与被复用资源池的硬件属性需要一致，两者共享起始地址及长
度；

- 输入长度需要小于等于被复用资源池长度；
- 其他泛用约束参考4.4.4.1.3 TBufPool。
