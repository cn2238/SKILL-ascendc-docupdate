...
- 无需指定源和目的位置
- non-inplace接口：将入队的LocalTensor地址从队列中取出赋值给新创建的
Tensor并返回
...
- inplace接口：通过出参的方式返回，可以减少Tensor反复创建的开销，具体
使用指导可参考2.9.9.5 如何使用Tensor原地操作提升算子性能。
...
- 需要指定源和目的位置
通过4.4.4.1.6 TQueBind绑定VECIN和VECOUT可实现VECIN和VECOUT内存复用，如下接口用于存在Vector计算的场景下实现复用，在出队时需要指定源和目的位置；不存在Vector计算的场景下可直接调用LocalTensor<T> DeQue()出队接口。
...
| --- | --- |
| T | Tensor的数据类型。 |
| srcUser | 用户指定队列的src position，当前只支持如下通路：GM->VECIN/<br>`Pos VECOUT->GM。`<br>dstUser 用户指定队列的dst position，当前只支持如下通路：GM->VECIN/<br>`Pos VECOUT->GM。` |
...
- non-inplace接口和指定源和目的位置的接口，需要将TQueBind的depth模板参数
设置为非零值；inplace接口，需要将TQueBind的depth模板参数设置为0。
...
que.EnQue<AscendC::TPosition::GM, AscendC::TPosition::VECIN, half>(tensor1);
 ...
 AscendC::LocalTensor<half> tensor2 = que.DeQue<AscendC::TPosition::GM, AscendC::TPosition::VECIN,
    half>();
       // inplace接口
 AscendC::TPipe pipe;
