...
具体的初始化函数代码如下：__aicore__ inline void Init(GM_ADDR x, GM_ADDR y, GM_ADDR z, AddCustomTilingData tiling)
...
对应的，每个核内搬入、搬出每个数据块时，需定位到每个数据块所在Global Memory上的内存偏移地址，因此在CopyIn和CopyOut函数内部使用DataCopy接口时，需增加每个数据块的地址偏移。Compute函数没有变化，与基础矢量算子相同。CopyIn函数实现代码如下：__aicore__ inline void CopyIn(int32_t progress)
...
CopyOut函数实现代码如下：__aicore__ inline void CopyOut(int32_t progress)
