...
| 参数名 | 描述 |
| --- | --- |
| engine | 引擎模式，参数取值分别为AIC、AIV。<br>`enum class EngineType : int32_t {` |
| AIC | = 1, // 仅AIC AIV = 2 // 仅AIV<br>`};` |
| funPtr | 函数指针，指定要执行的函数，函数签名和参数类型由class... Args决定。<br>`class... 可变参数模板，表示函数参数的类型列表，用于传递给funPtr。`<br>`Args` |
