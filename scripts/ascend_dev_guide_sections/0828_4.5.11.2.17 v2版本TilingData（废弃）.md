...
| hcom | Mc2HcommCfg类型。各通信域中每个通信任务的参数配置。在通信算子TilingData的定义中，根据各通信域中通信任务总个数，共需要定义mc2HcommCnt个Mc2HcommCfg结构体。例如：mc2HcommCnt=2，则需要 |

### 依次定义2个Mc2HcommCfg类型的参

数，自定义参数名，比如hcom1、hcom2。
