...
| 参数名称 | 输入/输出 | 含义 |
| --- | --- | --- |
| checkParams | 输入 | 用于配置对UB访问的检查行为，类型为CheckLocalMemoryIAParam。具体定义请参考${INSTALL_DIR}/include/ascendc/basic_api/interface/kernel_struct_mm.h，${INSTALL_DIR}请替换为CANN软件安装后文件存储路径。<br>参数说明请参考表4-449。 |

表4-449 CheckLocalMemoryIAParam 结构体内参数说明
...
| - 2： | 异常寄存器2。 |
| - 3： | 异常寄存器3。 |
| startAddr Check | 的起始地址，32B对齐，取值范围：startAddr∈[0, 65535]，默认值为0。比如，可通过LocalTensor.GetPhyAddr()/32来获取startAddr。 |
| endAddr Check | 的结束地址，32B对齐，取值范围：endAddr∈[0, 65535]。默认值为0。 |
| isScalarRe Check | 标量读访问。ad ●false：不开启，默认为false。 |
| - true： | 开启。 |
| isScalarW Check | 标量写访问。rite ●false：不开启，默认为false。 |
| - true： | 开启。 |
| isVectorRe Check | 矢量读访问。ad ●false：不开启，默认为false。 |
| - true： | 开启。 |
| isVectorW Check | 矢量写访问。rite ●false：不开启，默认为false。 |
| - true： | 开启。 |
| isMteRea Check Mte | 读访问。d ●false：不开启，默认为false。 |
| - true： | 开启。 |
| isMteWrit Check Mte | 写访问。e ●false：不开启，默认为false。 |
| - true： | 开启。 |
...
- startAddr/endAddr的单位是32B，check的范围不包含startAddr，包含
endAddr，即(startAddr，endAddr]。
