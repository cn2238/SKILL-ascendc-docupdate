...
| repeatPa | 输入 | 控制操作数地址步长的参数。4.8.5 BinaryRepeatParams类rams 型，包含操作数相邻迭代间相同datablock的地址步长，操作数同一迭代内不同datablock的地址步长等参数。相邻迭代间的地址步长参数说明请参考repeatStride；同一<br>迭代内DataBlock的地址步长参数说明请参考<br>dataBlockStride。 |
