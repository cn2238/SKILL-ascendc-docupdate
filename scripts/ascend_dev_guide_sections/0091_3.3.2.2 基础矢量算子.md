...
x（输入） (1, 2048) half ND y（输入） (1, 2048) half ND

z（输出） (1, 2048) half ND

核函数名称 add_custom

使用的主要接 DataCopy：数据搬移接口口Add：矢量基础算术接口

EnQue、DeQue等接口：Queue队列管理接口

### 算子实现文件 add_custom.cpp
...
constexpr int32_t TOTAL_LENGTH = 1 * 2048; // 数据总长__aicore__ inline void Init(GM_ADDR x, GM_ADDR y, GM_ADDR z)
