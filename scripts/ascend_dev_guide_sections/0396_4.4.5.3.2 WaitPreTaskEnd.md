...
Atlas 200I/500 A2 推理产品 √ 仅保证编译兼容，实际功能不生效。产品是否支持备注
...
在SuperKernel的子Kernel中调用，调用前的指令可以和前序其他的子Kernel实现并行，提升整体性能。如图4-23所示，SuperKernel按序调用子Kernel，为保证子Kernel之间数据互不干扰，会在子Kernel间插入算子间同步进行保序，子KernelN+1调用该接口之前的指令会和前序子KernelN实现并行。
...
图4-23 通过WaitPreTaskEnd 实现并行示意图
