...
ascendcPl 输入传入硬件平台的信息，PlatformAscendC定义请参见atform 4.6.2.1.2 构造及析构函数。

platform 输入传入硬件版本以及AI Core中各个硬件单元提供的内存大小。

PlatformInfo构造时通过4.6.2.1.2 构造及析构函数获取。PlatformInfo结构定义如下，socVersion通过4.6.2.1.4 GetSocVersion获取并透传，各类硬件存储空间大小通过4.6.2.1.9 GetCoreMemSize获取并透传。
...
不推荐通过直接填值构造PlatformInfo的方式调用构造函

```cpp
数，例如PlatformInfo(, 1024, 1024, ..);
```
