...
batchWriteI 输入通信任务信息的Global Memory地址。一组通信数据的相nfo 关信息必须按指定的格式保存，在执行通信任务时，可以同时指定多组通信任务信息，执行通信任务时批量发送数据。
...
- count：该通信任务发送的数据个数。
- dataType：该通信任务发送的数据类型，支持的类型可
参考HcclDataType参数说明。

- remoteRankId：该通信任务发送数据的目的卡卡号。

itemNum 输入批量任务的个数。该参数取值必须与batchWriteInfo中通信任务信息的组数一致。
