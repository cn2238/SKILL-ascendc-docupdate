...
| 参数名 | 输入/输出 | 描述 |
| --- | --- | --- |
| commit | 输入 | bool类型。参数取值如下：<br>- true：在调用Prepare接口时，Commit同步通知服务端<br>可以执行该通信任务。<br>- false：在调用Prepare接口时，不通知服务端执行该通<br>信任务。<br>表4-905 接口参数说明<br>### 参数名 输入/输出 描述 |
| \| 参数名 \| | 输入/输出 | \| 描述 \|<br>\| --- \| --- \| --- \| |
...
当通信数据量较大时，可以在Tiling流程中调用SetAicpuBlockDim接口来设置AI
```

CPU的核数。算子内部将自动在多个AI CPU核中选择最优的核进行通信，以实现更优的性能。建议将可调度的AI CPU核数设置为5。
