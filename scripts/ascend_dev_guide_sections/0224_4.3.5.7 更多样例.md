...
```cpp
 ...
 extern "C" __global__ __aicore__ void binary_scalar_trait_kernel(__gm__ uint8_t* src, __gm__ uint8_t*
dstGm)
  {
...
```cpp
#include "kernel_operator.h"
template <typename dst_T, typename fmap_T, typename weight_T, typename dstCO1_T, typename
bias_T> class KernelMatmul {
```

public:__aicore__ inline KernelMatmul(uint16_t mIn, uint8_t kIn, uint8_t nIn, bool initl1In, bool initl0In)
...
inQueueB1.AllocTensor<AscendC::TensorTrait<weight_T>>();
       if(initl1 == true) {
        ...
        AscendC::InitConstValue(b1Local, {static_cast<uint16_t>(k * n * sizeof(weight_T) / 32), 1, 0,
1});
      } else {
...
inQueueB2.AllocTensor<AscendC::TensorTrait<weight_T>>();
     ...
     loadL0BParams.dstGap = 0;
        if (initl0 == true) {
        AscendC::InitConstValue(b2Local, {static_cast<uint16_t>(k * n * sizeof(weight_T) / 512), 1, 0,
1});
      } else{
