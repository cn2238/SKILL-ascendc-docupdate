...
| pos | 输入 | 用于指定DataCopyPad接口搬运过程中从GM搬运数据到哪一个目的地址，目的地址通过逻辑位置来表达。默认值为TPosition::MAX，等效于TPosition::VECIN或<br>`TPosition::VECOUT。`<br>支持的取值为：<br>- TPosition::VECIN、TPosition::VECOUT、<br>`TPosition::MAX`<br>表4-292 参数说明<br>### 参数名 输入/输出 描述 |
| \| 参数名 \| | 输入/输出 | \| 描述 \|<br>\| --- \| --- \| --- \| |
| \| paddingVal \| | 输入 | \| DataCopyPad接口填充的数值，数据与DataCopyPad接口ue 搬运的数据类型一致。 \| |
