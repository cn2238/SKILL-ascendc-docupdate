...
matmul_tiling::DataType::DT_FLOAT);
 ...
 tiling.SetOrgShape(1024, 1024, 1024); // 设置原始输入M、N、K大小，单核Tiling与SetShape一致。若
Ka,Kb不等长时，设置tiling.SetOrgShape(1024, 1024, 1024, 1280)
...
```cpp
 ...
 bmmTiling.SetBatchInfoForNormal(2, 2, 64, 48, 32);
   // Layout类型为BSNGD、SBNGD、BNGS1S2时, 通过SetALayout、SetBLayout、SetCLayout设置A、B、C
// 矩阵的Layout轴信息
...
表4-652 MatmulApiTiling/MultiCoreMatmulTiling/BatchMatmulTiling 共有接口列表
...
| SetBatchInfoForNorma | 设置A/B矩阵的M/N/K轴信息，以及A/B矩阵各自的Batch l 数。 |
...
| SetMatmulConfigPara | 自定义设置MatmulConfig参数。ms接口功能 |
...
表4-653 MultiCoreMatmulTiling 其他接口

| 接口 | 功能 |
| --- | --- |
| SetDim | 设置多核Matmul时，可以参与运算的核数。 |
| SetSingleRa | 设置singleCoreM/singleCoreN/singleCoreK的最大值与最小值，单nge 位为元素个数。 |
| SetSingleSh | 设置Matmul单核计算的形状singleCoreM、singleCoreN、ape singleCoreK，单位为元素个数。 |
| GetSingleSh | 获取计算后的singleCoreM/singleCoreN/singleCoreK。ape |
| SetAlignSplit | 设置多核切分时singleCoreM/singleCoreN/singleCoreK的对齐值 |
| GetCoreNu | 获得多核切分后， 使用的blockDim。m |
| SetSplitK | 多核场景，使能切K轴。建议使用EnableMultiCoreSplitK接口。 |
| EnableMulti | 多核场景，使能切K轴。CoreSplitK |

表4-654 BatchMatmulTiling 其他接口

| 接口 | 功能 |
| --- | --- |
| GetCoreNu | 获得多核切分后， 使用的blockDim。m<br>需要包含的头文件 |
| - Matmul | 单核Tiling |
