...
```cpp
template <typename T>
  __aicore__ inline void InitBufHandle(T* bufPool, uint32_t index, TBufHandle bufhandle, uint32_t
 curPoolAddr, uint32_t len)
```
...
T bufPool的数据类型。表4-338 参数说明

### 参数名 输入/输 含义

称出

bufPool 输入用户自定义的TBufPool对象。

index 输入需要设置的内存块的偏移下标值，第一块为0，第二块为1，...，依次类推。

bufhan 输入需要设置的内存块指针，类型为TBufHandle(实际为dle uint8_t*)。

curPool 输入需要设置的内存块的地址。Addr

len 输入需要设置的内存块的大小，单位为bytes。
