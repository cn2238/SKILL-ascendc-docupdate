...
| loadDataP | 输入 | LoadData参数结构体，类型为LoadImageToLocalParams。arams 具体定义请参考${INSTALL_DIR}/include/ascendc/<br>`basic_api/interface/kernel_struct_mm.h，${INSTALL_DIR}`<br>请替换为CANN软件安装后文件存储路径。参数说明参考表4-424。 |

表4-424 LoadImageToLocalParams 结构体内参数说明
...
| horizStartP | 输入 | 加载图片在源图片上的水平起始地址，单位为像素，取值范os 围：horizStartPos∈[0, 4095]。默认为0。<br>注意：当输入图片为YUV420SP、XRGB8888， RGB888和YUV400格式时，该参数需要是偶数。 |
| vertStartPo | 输入 | 加载图片在源图片上的垂直起始地址，单位为像素，取值范s 围：vertStartPos∈[0, 4095]。默认为0。<br>注意：当输入图片为YUV420SP格式时，该参数需要是偶数。 |
| srcHorizSiz | 输入 | 源图像水平宽度，单位为像素，取值范围：srcHorizSize∈e [2, 4095]。<br>注意：当输入图片为YUV420SP格式时，该参数需要是偶数。 |
| topPadSize | 输入 | 目的图像顶部填充的像素数，取值范围：topPadSize∈[0,32]，默认为0。进行数据填充时使用，需要先调用 |

### SetAippFunctions通过AippPaddingParams配

置填充的数值，再通过topPadSize、botPadSize、leftPadSize、rightPadSize配置填充的大小范围。

botPadSize 输入目的图像底部填充的像素数，取值范围：botPadSize∈[0,32]，默认为0。

leftPadSize 输入目的图像左边填充的像素数，取值范围：leftPadSize∈[0,32]，默认为0。

rightPadSiz 输入目的图像右边填充的像素数，取值范围：rightPadSize∈[0,e 32]，默认为0。

sid 输入预留参数。为后续的功能做保留，开发者暂时无需关注，使用默认值即可。

### 约束说明

- 操作数地址对齐要求请参见通用地址对齐约束。
- 加载到dst的图片的大小加padding的大小必须小于等于所在存储空间的大小。
- 当通过4.4.9.1.17 SetAippFunctions配置padding模式为块填充模式或者镜像块
填充模式时，因为padding的数据来自于抠出的图片，左右padding的长度（leftPadSize、rightPadSize）必须小于或等于抠图的水平长度（horizSize），上下padding的长度（topPadSize、botPadSize）必须小于或等于抠图的垂直的长度（vertSize）。
...
>FetchEventID(AscendC::HardEvent::MTE1_MTE3));
   ...
   AscendC::GlobalTensor<uint8_t> fmGlobal;
    ...
    uint16_t horizSize = 32, vertSize = 32, horizStartPos = 0, vertStartPos = 0, srcHorizSize = 32, srcVertSize =
   32, leftPadSize = 0, rightPadSize = 0;
 ...
 extern "C" __global__ __aicore__ void load_image_simple_kernel(__gm__ uint8_t *fmGm, __gm__ uint8_t
```
