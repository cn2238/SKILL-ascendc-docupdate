...
template <typename T, bool highPrecision = false, bool highPerformance = false>
   __aicore__ inline void Gelu(const LocalTensor<T>& dstLocal, const LocalTensor<T>& srcLocal, const
   uint32_t dataSize)
```
...
template <typename T, bool highPrecision = false, bool highPerformance = false>
   __aicore__ inline void Gelu(const LocalTensor<T>& dstLocal, const LocalTensor<T>& srcLocal, const
 LocalTensor<uint8_t>& sharedTmpBuffer, const uint32_t dataSize)
```
...
| sharedTmpBuffe | 输入 | 临时缓存。r 类型为LocalTensor，支持的TPosition为VECIN/VECCALC/VECOUT。<br>用于接口内部复杂计算时存储中间变量，由开发者提供。<br>临时空间大小BufferSize的获取方式请参考4.5.3.3.4 GetGeluMaxMinTmpSize。 |
...
结果示例如下：输入数据(srcLocal):[-1.251 1.074 -6.137 -9.67 -5.066 -9.44 -3.588 -5.758 -7.484
