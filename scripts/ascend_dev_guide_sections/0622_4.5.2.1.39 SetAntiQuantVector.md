...
```cpp
 __aicore__ inline void SetAntiQuantVector(const LocalTensor<SrcT> &offsetTensor, const LocalTensor<SrcT>
&scaleTensor)
```
...
| 参数名 | 输入/输出 | 描述 |
| --- | --- | --- |
| offsetTens | 输入 | 伪量化运算时的参数向量，用于加。SrcT为A_TYPE中对应的or 数据类型。 |
| scaleTens | 输入 | 伪量化运算时的参数向量，用于乘。SrcT为A_TYPE中对应的or 数据类型。 |
