...
| vdeqInfo | 输入 | 存储量化tensor信息的数据结构，结构体内包含量化tensor |
...
- vdeqScale：float类型的数组，用于存储量化tensor中的
scale参数scale0-scale15。

- vdeqOffset：int16_t类型的数组，用于存储量化tensor
中的offset参数offset0-offset15。

- vdeqSignMode：bool类型的数组，用于存储量化tensor
中的signMode参数signMode0-signMode15。
