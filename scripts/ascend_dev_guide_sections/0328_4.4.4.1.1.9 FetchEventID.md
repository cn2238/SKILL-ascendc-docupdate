...
| 参数名 | 输入/输出 | 含义 |
| --- | --- | --- |
| evt | 输入 | HardEvent类型，硬件同步类型。<br>该类型的具体说明请参考4.4.5.1.3 SetFlag/WaitFlag(ISASI)中同步类型的说明。 |
...
```cpp
       AscendC::TEventID eventIdVToS = GetTPipePtr()->FetchEventID(AscendC::HardEvent::V_S); //需要插scalar等
