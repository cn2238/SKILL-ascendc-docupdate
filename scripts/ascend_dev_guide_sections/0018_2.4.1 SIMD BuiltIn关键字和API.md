...
表2-10 昇腾AI 处理器型号和__NPU_ARCH__的对应关系
...
度损失时使用CAST_RINT模式，在不涉及精度损失时不进行舍入#elif __NPU_ARCH__ == 2201
...
- ASCENDC_CUBE_ONLY
ASCENDC_CUBE_ONLY是通过C++宏实现的条件判断语句，用于在__aicore__修饰的函数中实现代码的条件编译。基于分离模式开发非融合算子时，在只有矩阵计算的算子场景下，可以通过设置ASCENDC_CUBE_ONLY，使能纯Cube模式完成Matmul计算，减少消息通信的性能开销，提升算子性能。
...
constexpr ●AscendC::AIC 常量值由框架自动设置，AIC核下，配置为int32_t AscendC::AIC，AIV核下，配置为 ●AscendC::AIV
...
```cpp
${INSTALL_DIR}请替换为CANN软件安装后文件存储路径。以root用户安装为例，则安装后文件
```

存储路径为：/usr/local/Ascend/cann。
