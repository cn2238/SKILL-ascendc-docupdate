...
pos 输入 A矩阵所在的buffer位置，可设置为：TPosition::GM,

```cpp
TPosition::VECOUT, TPosition::TSCM。
```

type 输入 A矩阵的数据格式，可设置为：CubeFormat::ND，

```cpp
CubeFormat::NZ，CubeFormat::VECTOR
```

dataType 输入 A矩阵的数据类型，可设置为：DataType::DT_FLOAT/
