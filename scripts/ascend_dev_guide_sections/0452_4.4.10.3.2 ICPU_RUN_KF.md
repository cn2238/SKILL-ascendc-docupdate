...
func 输入算子的kernel函数指针。

blkdim 输入算子的核心数，corenum。

... 输入所有的入参和出参，依次填入，当前参数个数限制为32个，超出32时会出现编译错误。
...
1. 步骤1 按需包含头文件，通过ASCENDC_CPU_DEBUG宏区分CPU和NPU侧需要包含的头文
件。
...
2. 步骤2 CPU侧运行验证。完成算子核函数CPU侧运行验证的步骤如下：
