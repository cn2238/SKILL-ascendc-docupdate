...
| shapeInfo | 输入 | 表示LayerNormGrad各个输入的数据排布格式Format。默认值表示输入的Format为ND。支持的取值为DataFormat::ND。LayerNormGradShapeInfo类型，具体定义如下。<br>`struct LayerNormGradShapeInfo {`<br>`DataFormat dataFormat = DataFormat::ND;`<br>`};` |
