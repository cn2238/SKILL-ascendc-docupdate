...
inputDy 输入源操作数，shape为[B, S, H]，LocalTensor数据结构的定义请参考4.3.1 LocalTensor。inputDy的数据类型需要与目的操作数保持一致，尾轴长度需要32B对齐。
...
inputVariance 输入方差，shape为[B, S]，LocalTensor数据结构的定义请参考4.3.1 LocalTensor。inputVariance的数据类型需要与目的操作数保持一致，尾轴长度需要32B对齐。需提前调用LayerNorm接口获取方差。

类型为LocalTensor，支持的TPosition为VECIN/VECCALC/VECOUT。

inputMean 输入均值，shape为[B, S]，LocalTensor数据结构的定义请参考4.3.1 LocalTensor。inputMean的数据类型需要与目的操作数保持一致，尾轴长度需要32B对齐。需提前调用LayerNorm接口获取均值。

类型为LocalTensor，支持的TPosition为VECIN/VECCALC/VECOUT。

inputGamma 输入源操作数，shape为[H]，LocalTensor数据结构的定义请参考4.3.1 LocalTensor。inputGamma的数据类型需要与目的操作数保持一致，尾轴长度需要32B对齐。

类型为LocalTensor，支持的TPosition为VECIN/VECCALC/VECOUT。

sharedTmpBuf 输入共享缓冲区，用于存放API内部计算产生的临时数fer 据。该方式开发者可以自行管理sharedTmpBuffer内存空间，并在接口调用完成后，复用该部分内存，内存不会反复申请释放，灵活性较高，内存利用率也较高。共享缓冲区大小的获取方式请参考4.5.4.4 LayerNormGrad Tiling。类型为LocalTensor，支持的TPosition为VECIN/VECCALC/VECOUT。
...
shapeInfo 输入表示LayerNormGrad各个输入的数据排布格式Format。默认值表示输入的Format为ND。支持的取值为DataFormat::ND。LayerNormGradShapeInfo类型，具体定义如下。
