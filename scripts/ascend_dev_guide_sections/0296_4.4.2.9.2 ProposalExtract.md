...
| modeNu | 输入 | 抽取位置参数，取值范围：modeNumber∈[0, 5]，int32_t mber 类型，仅限于以下配置：<br>- 0 – 从x1抽取<br>- 1 – 从y1抽取<br>- 2 – 从x2抽取<br>- 3 – 从y2抽取<br>- 4 – 从score抽取<br>- 5 – 从label抽取 |
...
- 用户需保证src中存储的proposal数目大于等于实际所需数目，否则会存在tensor
越界错误。

- 用户需保证dst中存储的元素大于等于实际所需数目，否则会存在tensor越界错
误。
