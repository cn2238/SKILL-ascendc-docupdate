...
repeatTi 输入重复迭代次数，int32_t类型，每次迭代完成16个Region me Proposals的元素抽取并排布到16个元素里，下次迭代跳至相邻的下一组16个Region Proposals和下一组16个元素。取值范围：repeatTime∈[0,255]。

modeNu 输入抽取位置参数，取值范围：modeNumber∈[0, 5]，int32_t mber 类型，仅限于以下配置：
