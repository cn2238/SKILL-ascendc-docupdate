...
设置C矩阵的位置，数据格式，数据类型等信息，这些信息需要和kernel侧的设置保持一致。函数原型int32_t SetCType(TPosition pos, CubeFormat type, DataType dataType)
...
pos 输入 C矩阵所在的buffer位置，可设置为：TPosition::GM,

```cpp
TPosition::VECIN。
```

type 输入 C矩阵的数据格式，可设置为：CubeFormat::ND，

```cpp
CubeFormat::NZ, CubeFormat::ND_ALIGN。
```

dataType 输入 C矩阵的数据类型，可设置为：DataType::DT_FLOAT/

```cpp
DataType::DT_FLOAT16/DataType::DT_BFLOAT16 /
