...
| 参数名 | 输入/输出 | 描述 |
| --- | --- | --- |
| pos | 输入 | C矩阵所在的buffer位置，可设置为：TPosition::GM,<br>`TPosition::VECIN。`<br>type 输入 C矩阵的数据格式，可设置为：CubeFormat::ND，<br>`CubeFormat::NZ, CubeFormat::ND_ALIGN。`<br>dataType 输入 C矩阵的数据类型，可设置为：DataType::DT_FLOAT/<br>`DataType::DT_FLOAT16/DataType::DT_BFLOAT16 /`<br>`DataType::DT_INT8/DataType::DT_INT32。`<br>返回值说明<br>- 1表示设置失败；0表示设置成功。 |
