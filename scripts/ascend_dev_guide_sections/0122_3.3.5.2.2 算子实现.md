...
| 项目 | 值 |
| --- | --- |
| 算子输入 | a [512, float16 ND 5120]<br>b [5120, float16 ND 640]<br>c [4096, float16 ND 640]<br>gather_out [4096, float16 ND 5120]<br>算子属性 group（char*），Host侧标识通信域的字符串，表示通信域名称。 |
...
AllGatherMatmulCustomTilingFunc传入，用于保存算子Tiling计算的上下文。在AllGatherMatmul算子的Tiling实现中，通过该上下文context获取计算Tiling所需要的输入输出shape、输入属性等参数，然后将Tiling结果（例如TilingKey、TilingData）保存至上下文中，供后续算子执行时使用。
