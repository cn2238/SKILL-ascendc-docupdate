...
Maxs 源操作数矢量内每个元素与标量相比，如果比标量大，则取源操作数值，比标量的值小，则取标量值。分类接口名功能描述
...
Compare（结果逐元素比较两个tensor大小，如果比较后的结果存放入寄存器） 为真，则输出结果的对应比特位为1，否则为0。
...
Copy VECIN、VECCALC、VECOUT之间的搬运指令，支持mask操作和DataBlock间隔操作。表4-5 资源管理API 列表
...
IBSet 当不同核之间操作同一块全局内存且可能存在读后写、写后读以及写后写等数据依赖问题时，通
...
DataCacheCleanAndInvalid 该接口用来刷新Cache，保证Cache的一致性。表4-8 系统变量访问API 列表
...
MetricsProfStop 设置性能数据采集信号停止，和MetricsProfStart配合使用。使用msProf工具进行算子上板调优时，可在kernel侧代码段前后分别调用MetricsProfStart和MetricsProfStop来指定需要调优的代码段范围。表4-11 工具函数接口列表
...
REGISTER_NONE_TILING 在Kernel侧使用标准C++语法自定义的TilingData结构体时，若用户不确定需要注册哪些结构体，可使用该接口告知框架侧需使用未注册的标准C+

- 语法来定义TilingData，并配套4.4.13.2
...
KERNEL_TASK_TYPE 设置某一个具体的tiling key对应的kernel type。表4-13 ISASI 接口列表

### 分类 接口名 功能描述

矢量计 VectorPadding 根据padMode（pad模式）与padSide（pad方算向）对源操作数按照datablock进行填充操作。

BilinearInterpolati 双线性插值操作，分为垂直迭代和水平迭代。on
...
LoadData LoadData包括Load2D和Load3D数据加载功能。分类接口名功能描述
...
SetLoadDataBound 设置Load3D时A1/B1边界值。ary

SetLoadDataRepea 用于设置Load3Dv2接口的repeat参数。设置t repeat参数后，可以通过调用一次Load3Dv2接口完成多个迭代的数据搬运。

SetLoadDataPaddi 设置padValue，用于Load3Dv1/Load3Dv2。ngValue
...
DataSyncBarrier 用于阻塞后续的指令执行，直到所有之前的内存

### 访问指令（需要等待的内存位置可通过参数控
...
Swish 神经网络中的Swish激活函数。表4-18 归约操作API 列表
...
DropOut 提供根据MaskTensor对源操作数进行过滤的功能，得到目的操作数。表4-21 张量变换API 列表
...
PlatformAscendCManager 基于Kernel Launch算子工程，通过基础调用（Kernel Launch）方式调用算子的场景下，可能需要获取硬件平台相关信息，比如获取硬件平台的核数。PlatformAscendCManager类提供获取平台信息的功能。表4-28 原型注册与管理API 列表
...
ContextBuilder ContextBuilder类提供一系列的API接口，支持手动构造TilingContext类来验证Tiling函数以及KernelContext类用于TilingParse函数的验证。表4-31 Tiling 模板编程API 列表

### 接口名 功能描述

### 模板参数定义 通过该类接口进行模板参数

### ASCENDC_TPL_ARGS_DECL和模板参数组合
