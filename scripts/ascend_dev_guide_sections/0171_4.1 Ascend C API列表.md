...
运行时编译及日志输出等功能，支持开发者高效实现算子开发与性能优化。基础数据结构

表4-1 基础数据结构列表

| 接口名 | 功能描述 |
| --- | --- |
| LocalTensor LocalTensor | 用于存放AI Core中Local Memory（内部存储）的数据，支持逻辑位置TPosition为VECIN、VECOUT、VECCALC、A1、A2、B1、B2、CO1、CO2。 |
| GlobalTensor GlobalTensor | 用来存放Global Memory（外部存储）的全局数据。 |
| Coordinate Coordinate | 本质上是一个元组（tuple），用于表示张量在不同维度的位置信息，即坐标值。 |
| Layout Layout<Shape, Stride> | 数据结构是描述多维张量内存布局的基础模板类，通过编译时的形状（Shape）和步长（Stride）信息，实现逻辑坐标空间到一维内存地址空间的映射，为复杂张量操作和硬件优化提供基础支持。 |
| TensorTrait TensorTrait | 数据结构是描述Tensor相关信息的基础模板类，包含Tensor的数据类型、逻辑位置和Layout内存布局。<br>基础API |

表4-2 标量计算API 列表

| 接口名 | 功能描述 |
| --- | --- |
| ScalarGetCountOfValue | 获取一个uint64_t类型数字的二进制中0或者1的个数。接口名功能描述 |
...
| ToBfloat16 float | 类型标量数据转换成bfloat16_t类型标量数据。 |
| ToFloat bfloat16_t | 类型标量数据转换成float类型标量数据。 |
...
| 基础算术 | Maxs | 源操作数矢量内每个元素与标量相比，如果比标量大，则取源操作数值，比标量的值小，则取标量值。分类接口名功能描述 |
...
| 逻辑计算 | ShiftLeft | 对源操作数中的每个元素进行左移操作，左移的位数由输入参数scalarValue决定。 |
| 逻辑计算 | ShiftRight | 对源操作数中的每个元素进行右移操作，右移的位数由输入参数scalarValue决定。 |
| 复合计算 | Axpy | 源操作数中每个元素与标量求积后和目的操作数中的对应元素相加。 |
| 复合计算 | CastDeq | 对输入做量化并进行精度转换。 |
| 复合计算 | AddRelu | 按元素求和，结果和0对比取较大值。 |
| 复合计算 | AddReluCast | 按元素求和，结果和0对比取较大值，并根据源操作数和目的操作数Tensor的数据类型进行精度转换。 |
| 复合计算 | AddDeqRelu | 依次计算按元素求和、结果进行deq量化后再进行relu计算（结果和0对比取较大值）。 |
| 复合计算 | SubRelu | 按元素求差，结果和0对比取较大值。 |
| 复合计算 | SubReluCast | 按元素求差，结果和0对比取较大值，并根据源操作数和目的操作数Tensor的数据类型进行精度转换。 |
| 复合计算 | MulAddDst | 按元素将src0Local和src1Local相乘并和dstLocal相加，将最终结果存放进dstLocal中。 |
| 复合计算 | MulCast | 按元素求积，并根据源操作数和目的操作数Tensor的数据类型进行精度转换。 |
| 复合计算 | FusedMulAdd | 按元素将src0Local和dstLocal相乘并加上src1Local，最终结果存放入dstLocal。 |
| FusedMulAddRel | 按元素将src0Local和dstLocal相乘并加上u | src1Local，将结果和0作比较，取较大值，最终结果存放进dstLocal中。 |
| 比较与选 | Compare | 逐元素比较两个tensor大小，如果比较后的结果择为真，则输出结果的对应比特位为1，否则为0。分类接口名功能描述 |
| 比较与选 | Compare（结果逐元素比较两个tensor大小，如果比较后的结果存放入寄存器） | 为真，则输出结果的对应比特位为1，否则为0。 |

Compare接口需要mask参数时，可以使用此接口。计算结果存放入寄存器中。
...
精度转换 Cast 根据源操作数和目的操作数Tensor的数据类型进指令行精度转换。
...
WholeReduceMa 每个repeat内所有数据求最大值以及其索引x index。

WholeReduceMi 每个repeat内所有数据求最小值以及其索引n index。

WholeReduceSu 每个repeat内所有数据求和。m
...
RepeatReduceSu 每个repeat内所有数据求和。和4.4.2.6.6 m WholeReduceSum接口相比，不支持mask逐bit模式。建议使用功能更全面的4.4.2.6.6 WholeReduceSum接口。分类接口名功能描述
...
数据分散/ Gather 给定输入的张量和一个地址偏移张量，Gather指数据收集令根据偏移地址将输入张量按元素收集到结果张量中。
...
| Copy VECIN、VECCALC、VECOUT | 之间的搬运指令，支持mask操作和DataBlock间隔操作。 |

表4-5 资源管理API 列表

| 接口名 | 功能描述 |
| --- | --- |
| TPipe TPipe | 是用来管理全局内存等资源的框架。通过TPipe类提供的接口可以完成内存等资源的分配管理操作。 |
| GetTPipePtr | 获取框架当前管理全局内存的TPipe指针，用户获取指针后，可进行TPipe相关的操作。 |
| TBufPool TPipe | 可以管理全局内存资源，而TBufPool可以手动管理或复用Unified Buffer/L1 Buffer物理内存，主要用于多个stage计算中Unified Buffer/L1 Buffer物理内存不足的场景。 |
...
| 接口名 | 功能描述 |
| --- | --- |
| TQueSync TQueSync | 类提供同步控制接口，开发者可以使用这类API来自行完成同步控制。接口名功能描述 |
| IBSet | 当不同核之间操作同一块全局内存且可能存在读后写、写后读以及写后写等数据依赖问题时，通<br>过调用该函数来插入同步语句来避免上述数据依 |

赖时可能出现的数据读写错误问题。调用IBSet设置某一个核的标志位，与IBWait成对出现配合使用，表示核之间的同步等待指令，等待某一个核操作完成。

IBWait 当不同核之间操作同一块全局内存且可能存在读后写、写后读以及写后写等数据依赖问题时，通

### 过调用该函数来插入同步语句来避免上述数据依

赖时可能出现的数据读写错误问题。IBWait与IBSet成对出现配合使用，表示核之间的同步等待指令，等待某一个核操作完成。

SyncAll 当不同核之间操作同一块全局内存且可能存在读后写、写后读以及写后写等数据依赖问题时，通

### 过调用该函数来插入同步语句来避免上述数据依

赖时可能出现的数据读写错误问题。目前多核同步分为硬同步和软同步，硬件同步是利用硬件自带的全核同步指令由硬件保证多核同步，软件同步是使用软件算法模拟实现。

InitDetermineComputeWorks 初始化GM共享内存的值，完成初始化后才可以pace 调用4.4.5.2.8 WaitPreBlock和4.4.5.2.7 NotifyNextBlock。
...
| InitSocState | 由于AI Core上存在一些全局状态，如原子累加状态、Mask模式等，在实际运行中，这些值可以被前序执行的算子修改而导致计算出现不符合预期的行为，在静态Tensor编程的场景中用户必须在 |
| Kernel | 入口处调用此函数来初始化AI Core状 |
...
| ascendc_assert ascendc_assert | 提供了一种在CPU/NPU域实现断言功能的接口。当断言条件不满足时，系统会输出断言信息并格式化打印在屏幕上。 |
| assert | 基于算子工程开发的算子，可以使用该接口实现CPU/NPU域assert断言功能。接口名功能描述 |
...
| SetKernelMode CPU | 调测时，设置内核模式为单AIV模式，单AIC模式或者MIX模式，以分别支持单AIV矢量算子，单AIC矩阵算子，MIX混合算子的CPU调试。 |
| TRACE_START | 通过CAModel进行算子性能仿真时，可对算子任意运行阶段打点，从而分析不同指令的流水图，以便进一步性能调优。 |
| 用于表示起始位置打点，一般与4.4.10.4.2 | TRACE_STOP配套使用。 |
| TRACE_STOP | 通过CAModel进行算子性能仿真时，可对算子任意运行阶段打点，从而分析不同指令的流水图，以便进一步性能调优。 |
| 用于表示终止位置打点，一般与4.4.10.4.1 | TRACE_START配套使用。 |
...
| 接口名 | 功能描述 |
| --- | --- |
| Async Async | 提供了一个统一的接口，用于在不同模式下（AIC或AIV）执行特定函数，从而避免代码中直接的硬件条件判断（如使用ASCEND_IS_AIV或ASCEND_IS_AIC）。 |
| GetTaskRatio | 适用于Cube/Vector分离模式，用来获取Cube/Vector的配比。 |

表4-12 Kernel Tiling 接口列表

| 接口名 | 功能描述 |
| --- | --- |
| GET_TILING_DATA | 用于获取算子kernel入口函数传入的tiling信息，并填入注册的Tiling结构体中，此函数会以宏展开的方式进行编译。如果用户注册了多个TilingData结构体，使用该接口返回默认注册的结构体。 |
| GET_TILING_DATA_WITH_ST | 使用该接口指定结构体名称，可获取指定的tiling RUCT 信息，并填入对应的Tiling结构体中，此函数会以宏展开的方式进行编译。 |
...
| REGISTER_TILING_FOR_TILIN | 用于在kernel侧注册与TilingKey相匹配的GKEY TilingData自定义结构体；该接口需提供一个逻辑表达式，逻辑表达式以字符串“TILING_KEY_VAR”代指实际TilingKey，表达TIlingKey所满足的范围。 |
| REGISTER_NONE_TILING | 在Kernel侧使用标准C++语法自定义的TilingData结构体时，若用户不确定需要注册哪些结构体，可使用该接口告知框架侧需使用未注册的标准C+<br>- 语法来定义TilingData，并配套4.4.13.2 |
| GET_TILING_DATA_WITH_STRUCT，4.4.13.3 GET_TILING_DATA_MEMBER，4.4.13.4 GET_TILING_DATA_PTR_WITH_STRUCT | 来获取对应的TilingData。 |
| KERNEL_TASK_TYPE_DEFAUL | 设置全局默认的kernel type，对所有的tiling key T 生效。 |
...
| 分类 | 接口名 | 功能描述 |
| --- | --- | --- |
| 矢量计 | VectorPadding | 根据padMode（pad模式）与padSide（pad方算向）对源操作数按照datablock进行填充操作。 |
| 矢量计 | BilinearInterpolati | 双线性插值操作，分为垂直迭代和水平迭代。on |
| 矢量计 | GetCmpMask | 获取Compare（结果存入寄存器）指令的比较结果。 |
| 矢量计 | SetCmpMask | 为Select不传入mask参数的接口设置比较寄存器。 |
| 矢量计 | GetAccVal | 获取ReduceSum（针对tensor前n个数据计算）接口的计算结果。 |
| GetReduceMaxMin | 获取ReduceMax、ReduceMin连续场景下的最Count | 大/最小值以及相应的索引值。 |
...
| MrgSort4 | Sort32 | 排序函数，一次迭代可以完成32个数的排序。 |
...
| GetMrgSortResult | Gatherb | 给定一个输入的张量和一个地址偏移张量，Gatherb指令根据偏移地址将输入张量收集到结果张量中。 |
| GetMrgSortResult | Scatter | 给定一个连续的输入张量和一个目的地址偏移张量，Scatter指令根据偏移地址生成新的结果张量后将输入张量分散到结果张量中。 |
| 数据搬 | DataCopyPad | 该接口提供数据非对齐搬运的功能。运分类接口名功能描述 |
| 数据搬 | SetPadValue | 设置DataCopyPad接口填充的数值。 |
| 矩阵计 | Mmad | 完成矩阵乘加操作。算MmadWithSparse 完成矩阵乘加操作，传入的左矩阵A为稀疏矩阵， 右矩阵B为稠密矩阵。 |
| SetHF32Mode | 此接口同4.4.9.2.5 | SetHF32TransMode与4.4.9.2.3 SetMMLayoutTransform一样，都用于设置寄存器的值。SetHF32Mode接口用于设置MMAD的HF32模式。 |
| SetHF32TransMode | 此接口同4.4.9.2.4 | SetHF32Mode与4.4.9.2.3 SetMMLayoutTransform一样，都用于设置寄存器的值。SetHF32TransMode用于设置MMAD的HF32取整模式，仅在MMAD的HF32模式生效时有效。 |
| SetMMLayoutTrans | 此接口同SetHF32Mode与SetHF32TransMode | form 一样，都用于设置寄存器的值，其中SetMMLayoutTransform接口用于设置MMAD的M/N方向。 |
| SetMMLayoutTrans | Conv2D | 计算给定输入张量和权重张量的2-D卷积，输出结果张量。Conv2d卷积层多用于图像识别，使用过滤器提取图像中的特征。 |
| SetMMLayoutTrans | Gemm | 根据输入的切分规则，将给定的两个输入张量做矩阵乘，输出至结果张量。将A和B两个输入矩阵乘法在一起，得到一个输出矩阵C。 |
| SetMMLayoutTrans | SetFixPipeConfig | DataCopy（CO1->GM、CO1->A1）过程中进行随路量化时，通过调用该接口设置量化流程中tensor量化参数。 |
| SetFixpipeNz2ndFl | DataCopy（CO1->GM、CO1->A1） | 过程中进行ag 随路格式转换（NZ2ND）时，通过调用该接口设置NZ2ND相关配置。 |
| SetFixpipePreQuan | DataCopy（CO1->GM、CO1->A1） | 过程中进行tFlag 随路量化时，通过调用该接口设置量化流程中scalar量化参数。 |
| SetFixpipePreQuan | SetFixPipeClipRelu | DataCopy（CO1->GM）过程中进行随路量化后，通过调用该接口设置ClipRelu操作的最大值。 |
| SetFixpipePreQuan | SetFixPipeAddr | DataCopy（CO1->GM）过程中进行随路量化后，通过调用该接口设置element-wise操作时LocalTensor的地址。 |
| SetFixpipePreQuan | InitConstValue | 初始化LocalTensor（TPosition为A1/A2/B1/B2）为某一个具体的数值。 |
| SetFixpipePreQuan | LoadData | LoadData包括Load2D和Load3D数据加载功能。分类接口名功能描述 |
| LoadDataWithTran | 该接口实现带转置的2D格式数据从A1/B1到spose | A2/B2的加载。 |
...
| LoadImageToLocal | LoadUnZipIndex | 加载GM上的压缩索引表到内部寄存器。 |
| LoadImageToLocal | LoadDataUnzip | 将GM上的数据解压并搬运到A1/B1/B2上。 |
| LoadDataWithSpar | 用于搬运存放在B1里的512B的稠密权重矩阵到B2 | se 里，同时读取128B的索引矩阵用于稠密矩阵的稀疏化。 |
| LoadDataWithSpar | SetFmatrix | 用于调用Load3Dv1/Load3Dv2时设置FeatureMap的属性描述。 |
| LoadDataWithSpar | SetLoadDataBound | 设置Load3D时A1/B1边界值。ary |
| SetLoadDataRepea | 用于设置Load3Dv2接口的repeat参数。设置t | repeat参数后，可以通过调用一次Load3Dv2接口完成多个迭代的数据搬运。 |
| SetLoadDataRepea | SetLoadDataPaddi | 设置padValue，用于Load3Dv1/Load3Dv2。ngValue |
| Fixpipe | 矩阵计算完成后，对结果进行处理，例如对计算结果进行量化操作，并把数据从CO1搬迁到Global | Memory中。 |
| 同步控 | SetFlag/WaitFlag | 同一核内不同流水线之间的同步指令。具有数据制依赖的不同流水指令之间需要插此同步。 |
| 同步控 | PipeBarrier | 阻塞相同流水，具有数据依赖的相同流水之间需要插此同步。 |
| 同步控 | DataSyncBarrier | 用于阻塞后续的指令执行，直到所有之前的内存<br>访问指令（需要等待的内存位置可通过参数控 |
...
缓存处 ICachePreLoad 从指令所在DDR地址预加载指令到ICache中。理GetICachePreloadS 获取ICACHE的PreLoad的状态。tatus分类接口名功能描述

系统变 GetProgramCounte 获取程序计数器的指针，程序计数器用于记录当量访问 r 前程序执行的位置。
...
原子操 SetAtomicMax 原子操作函数，设置后续从VECOUT传输到GM的作数据是否执行原子比较，将待拷贝的内容和GM已有内容进行比较，将最大值写入GM。

SetAtomicMin 原子操作函数，设置后续从VECOUT传输到GM的数据是否执行原子比较，将待拷贝的内容和GM已有内容进行比较，将最小值写入GM。

SetStoreAtomicConf 设置原子操作使能位与原子操作类型。

ig

GetStoreAtomicCo 获取原子操作使能位与原子操作类型的值。

nfig

调试接 CheckLocalMemory 监视设定范围内的UB读写行为，如果监视到有设口 IA 定范围的读写行为则会出现EXCEPTION报错，未监视到设定范围的读写行为则不会报错。

Cube分 CubeResGroupHan CubeResGroupHandle用于在分离模式下通过软组管理 dle 同步控制AIC和AIV之间进行通讯，实现AI Core计算资源分组。
...
| Normalize LayerNorm | 中，已知均值和方差，计算shape为[A，R]的输入数据的标准差的倒数rstd和归一化输出y。 |
| RmsNorm | 实现对shape大小为[B，S，H]的输入数据的RmsNorm归一化。 |
| WelfordUpdate | 实现Welford算法的前处理。接口名功能描述 |
...
| Gelu GELU | 是一个重要的激活函数，其灵感来源于relu和dropout，在激活中引入了随机正则的思想。 |
...
| SoftmaxFlash SoftMax | 增强版本，除了可以对输入tensor做softmaxflash计算，还可以根据上一次softmax计算的sum和max来更新本次的softmax计算结果。 |
| SoftmaxFlashV2 SoftmaxFlash | 增强版本，对应FlashAttention-2算法。 |
| SoftmaxFlashV3 SoftmaxFlash | 增强版本，对应Softmax PASA算法。 |
...
| 接口名 | 功能描述 |
| --- | --- |
| Matmul Matmul | 矩阵乘法的运算。 |

表4-24 HCCL 通信类API 列表

| 接口名 | 功能描述 |
| --- | --- |
| HCCL | 通信类在AI Core侧编排集合通信任务。 |
...
| Conv3DBackpropFilter | 卷积的反向运算，求解权重的反向传播误差。Utils API |
...
表4-29 Tiling 数据结构注册API 列表

| 接口名 | 功能描述 |
| --- | --- |
| TilingData | 结构定义定义一个TilingData的类，添加所需的成员变量（TilingData字段），用于保存所需TilingData参数。完成该TilingData类的定义后，该类通过继承TilingDef类（用来存放、处理用户自定义Tiling结<br>构体成员变量的基类）提供TilingData字段设置、 |

序列化和保存等接口。

TilingData结构注册注册定义的TilingData结构体并和自定义算子绑定。

表4-30 Tiling 调测API 列表

| 接口名 | 功能描述 |
| --- | --- |
| OpTilingRegistry OpTilingRegistry | 类属于context_ascendc命名空间，主要用于加载Tiling实现的动态库，并获取算子的Tiling函数指针以进行调试和验证。 |
| ContextBuilder ContextBuilder | 类提供一系列的API接口，支持手动构造TilingContext类来验证Tiling函数以及KernelContext类用于TilingParse函数的验证。 |

表4-31 Tiling 模板编程API 列表

| 接口名 | 功能描述 |
| --- | --- |
| 模板参数定义 | 通过该类接口进行模板参数 |
| ASCENDC_TPL_ARGS_DECL | 和模板参数组合 |
| ASCENDC_TPL_ARGS_SEL（ | 即可使用的模板）的定义。 |
| GET_TPL_TILING_KEY Tiling | 模板编程时，开发者通过调用此接口自动生成TilingKey。该接口将传入的模板参数通过定义的位宽，转成二进制，按照顺序组合后转成uint64数值，即TilingKey。 |
| ASCENDC_TPL_SEL_PARAM Tiling | 模板编程时，开发者通过调用此接口自动生成并配置TilingKey。 |

表4-32 Tiling 下沉API 列表

| 接口名 | 功能描述 |
| --- | --- |
| DEVICE_IMPL_OP_OPTILIN | 在Tiling下沉场景中，该宏定义用于生成Tiling下沉G 的注册类，再通过调用注册类的成员函数来注册需要下沉的Tiling函数。 |
...
| aclrtcGetCompileLogSize | 获取编译日志的大小。用于在4.6.8.8 aclrtcGetCompileLog获取日志内容时分配对应大小的内存空间。 |
