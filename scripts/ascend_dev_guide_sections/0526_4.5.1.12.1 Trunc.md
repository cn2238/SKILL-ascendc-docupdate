...
### GetTruncMaxMinTmpSize中提供的GetTruncMaxMinTmpSize接口获取需
...
| sharedTmpBuf | 输入 | 临时缓存。fer 类型为LocalTensor，支持的TPosition为VECIN/VECCALC/VECOUT。<br>用于Trunc内部复杂计算时存储中间变量，由开发者提供。<br>临时空间大小BufferSize的获取方式请参考4.5.1.12.2 GetTruncMaxMinTmpSize。 |
...
结果示例如下：输入数据(srcLocal): [ 0.5317103 -6.37912032 5.53408647 ... 11.11059642 -11.67860335]输出数据(dstLocal): [ 0.0 -6.0 5.0 ... 11.0 -11.0 ]
