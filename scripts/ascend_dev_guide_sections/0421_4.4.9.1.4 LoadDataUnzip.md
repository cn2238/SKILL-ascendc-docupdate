...
dst 输出目的操作数，类型为LocalTensor，支持的TPosition为A1/B1/B2。LocalTensor的起始地址需要保证：TPosition为A1/B1时，32字节对齐；TPosition为B2时，512B对齐。支持的数据类型为：int8_t。

### 参数名称 输入/ 含义

### 输出

src 输入源操作数，类型为GlobalTensor。数据类型需要与dst保持一致。
