...
| dst | 输出目的操作数，类型为LocalTensor。<br>数据连续排列顺序由目的操作数所在TPosition决定，具体约束如下：<br>- A2：ZZ格式；对应的分形大小为16 * (32B /<br>`sizeof(T))。`<br>- B2：ZN格式；对应的分形大小为 (32B / sizeof(T)) *<br>16。<br>- A1/B1：无格式要求，一般情况下为NZ格式。NZ格式<br>下，对应的分形大小为16 * (32B / sizeof(T))。 |
