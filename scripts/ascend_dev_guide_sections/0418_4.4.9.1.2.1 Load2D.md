...
- A2：ZZ格式；对应的分形大小为16 * (32B /
sizeof(T))。

- B2：ZN格式；对应的分形大小为 (32B / sizeof(T)) *
16。

- A1/B1：无格式要求，一般情况下为NZ格式。NZ格式
下，对应的分形大小为16 * (32B / sizeof(T))。

src 输入源操作数，类型为LocalTensor或GlobalTensor。

数据类型需要与dst保持一致。

loadDataPa 输入 LoadData参数结构体，类型为：rams ●LoadData2DParams，具体参考表4-388。
