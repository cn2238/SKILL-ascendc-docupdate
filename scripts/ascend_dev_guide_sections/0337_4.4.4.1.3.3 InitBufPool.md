...
表4-310 InitBufPool(T& bufPool, uint32_t len) 原型定义参数说明

### 参数名 输入/输 含义

称出

bufPool 输入新划分的资源池，类型为TBufPool。

len 输入新划分资源池长度，单位为字节，非32字节对齐会自动向上补齐至32字节对齐。表4-311 InitBufPool(T& bufPool, uint32_t len, U& shareBuf) 原型定义参数说明
...
len 输入新划分资源池长度，单位为字节，非32字节对齐会自动向上补齐至32字节对齐。

shareBu 输入被复用资源池，类型为TBufPool，新划分资源池与被复用资源f 池共享起始地址及长度。

### 约束说明

1. 新划分的资源池与被复用资源池的物理内存需要一致，两者共享起始地址及长
度；

2. 输入长度需要小于等于被复用资源池长度；
3. 其他泛用约束参考4.4.4.1.3 TBufPool；
...
len 输入每个内存块的大小，单位为Bytes，非32Bytes对齐会自动向上补齐至32Bytes对齐
