...
| 参数名 | 输入/输出 | 含义 |
| --- | --- | --- |
| bufPool | 输入 | 新划分的资源池，类型为TBufPool。<br>len 输入新划分资源池长度，单位为字节，非32字节对齐会自动向上补齐至32字节对齐。 |

表4-311 InitBufPool(T& bufPool, uint32_t len, U& shareBuf) 原型定义参数说明
...
| --- | --- | --- |
| bufPool | 输入 | 新划分的资源池，类型为TBufPool。<br>len 输入新划分资源池长度，单位为字节，非32字节对齐会自动向上补齐至32字节对齐。 |
| shareBu | 输入 | 被复用资源池，类型为TBufPool，新划分资源池与被复用资源f 池共享起始地址及长度。 |
