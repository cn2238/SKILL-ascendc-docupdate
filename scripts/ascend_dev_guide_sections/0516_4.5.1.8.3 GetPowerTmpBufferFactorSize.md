...
| 参数名 | 输入/输出 | 功能 |
| --- | --- | --- |
| baseIsTens | 输入 | 底数是Tensor输入true，否则输入false or<br>expIsTensor 输入指数是Tensor输入true，否则输入false<br>typeIsInt 输入数据类型是int时输入true，否则输入false<br>typeSize 输入输入的数据类型大小，单位为字节。比如输入的数据类型为half，此处应传入2。 |
