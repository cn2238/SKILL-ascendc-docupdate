...
| 参数名 | 描述 |
| --- | --- |
| hard | 用于表示数据的物理位置，Hardware枚举类型，定义如下，合法位置为：UB、L1、L0A、L0B、L0C、BIAS、FIXBUF。<br>`enum class Hardware : uint8_t {`<br>`GM, // Global Memory`<br>`UB, // Unified Buffer`<br>`L1, // L1 Buffer`<br>`L0A, // L0A Buffer`<br>`L0B, // L0B Buffer`<br>`L0C, // L0C Buffer`<br>`BIAS, // BiasTable Buffer`<br>`FIXBUF, // Fixpipe Buffer`<br>`MAX };` |
