...
| isReuseSour | 输入 | 是否复用源操作数的内存空间，与Normalize接口一致。ce<br>isComputeR 输入是否计算rstd。该参数的取值只支持true。std<br>isOnlyOutp 输入是否只输出y，不输出标准差的倒数rstd。当前该参数仅支持ut 取值为false，表示y和rstd的结果全部输出。 |
