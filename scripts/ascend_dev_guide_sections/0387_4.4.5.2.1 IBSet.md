...
```cpp
 template <bool isAIVOnly = true>
   __aicore__ inline void IBSet(const GlobalTensor<int32_t>& gmWorkspace, const LocalTensor<int32_t>&
...
ubWorkspac 输入存储当前核状态的公共缓存。e 类型为LocalTensor，支持的TPosition为VECIN/VECCALC/VECOUT。

blockIdx 输入表示等待核的idx号，取值范围：[0, 核数-1]。
...
- 注意：如果是AIVOnly模式，核数 = GetBlockNum()；如果是MIX模式，核数 =
GetBlockNum() * 2；
...
```cpp
 extern "C" __global__ __aicore__ void add_simple_kernel(__gm__ uint8_t* x, __gm__ uint8_t* y, __gm__
   uint8_t* sync,
__gm__ uint8_t* z)
      {
  KernelAdd op;
...
输入数据:x: [1,1,1,1,1,...,1] // 512个1 y: [1,1,1,1,1,...,1] // 512个1
