...
AI Core 昇腾AI处理器的计算核，负责执行矩阵、矢量计算密集的任务。术语/缩略含义语
...
Block Block在不同场景下具有多种含义，通常情况下指AI Core的逻辑核。

典型场景有：

- AI Core逻辑核：一个Block表示一个AI Core的逻辑核，其BlockID
是以0为起始的逻辑编号。

- DataBlock：一个DataBlock表示一条NPU矢量计算指令处理的数据
单元，大小通常为32字节，一条指令可同时处理多个DataBlock。
...
BiasTable 偏置存储，AI Core内部物理存储单元，通常用于存储矩阵计算所需的Buffer Bias（偏置）数据，与逻辑内存AscendC::TPosition::C2相对应。
...
C2PIPE2G AscendC::TPosition::C2PIPE2GM代表设备上用于矩阵计算的逻辑内M 存，用于存放量化参数，物理存储对应AI Core的Fixpipe Buffer。

Cache Line 缓存（DCache、ICache、L2 Cache）中的最小数据单位。

Core 拥有独立Scalar计算单元的计算核，Scalar计算单元承担了核内的指令发射等功能，也称之为核内的调度单元。术语/缩略含义语
...
DataBlock 矢量计算指令单次Repeat内DataBlock的间隔大小，即下次处理的起Stride 始数据地址与本次处理的起始数据地址之间的DataBlock个数。
...
DoubleBuf 双缓冲，并行领域常用的优化方式，通过创建多个持有数据的缓冲区fer/DB （Buffer）提高数据处理的并行性。

Elementwi 元素级操作是对张量的每个元素独立进行的操作。每个元素的结果仅se 依赖于对应的输入元素。

Fixpipe AI Core中负责将矩阵计算结果从L0C Buffer搬运到Global Memory或L1 Buffer的单元，搬运过程中随路完成量化、激活等操作。术语/缩略含义语

Fixpipe AI Core内部物理存储单元，通常用于存储Fixpipe搬运过程中所需的量Buffer 化参数等数据，与逻辑内存AscendC::TPosition::C2PIPE2GM相对应。

Global 设备端的主内存，AI Core的外部存储，用于存储大规模数据，但需要Memory/G 优化访问模式以提升性能。M

GlobalTens 存放Global Memory全局数据的Tensor。or
...
Kernel 将kernel程序提交至硬件进行启动执行的过程。Launch
...
Local AI Core的内部存储，包括L1 Buffer、L0A Buffer、L0B Buffer、L0C Memory Buffer、Unified Buffer等存储单元。术语/缩略含义语

LocalTenso 存放AI Core中Local Memory本地数据的Tensor。r
...
MTE2 Memory Transfer Engine 2，AI Core的数据传递引擎，负责将数据从GM搬运到L1 Buffer、L0A Buffer、L0B Buffer、Unified Buffer等。
...
Repeat 矢量计算指令循环执行时，下一次Repeat起始数据地址与当前Repeat Stride 起始数据地址之间的DataBlock个数。术语/缩略含义语

Repeat 矢量计算指令循环执行的次数。Times
...
VECOUT Vector Output，AscendC::TPosition::VECOUT代表设备上用于矢量计算的逻辑内存，用于存放矢量计算的输出数据，物理存储对应AI Core的Unified Buffer。术语/缩略含义语

Vector AI Core上的Vector计算单元，负责执行矢量运算。其算力低于Cube，但灵活度高于Cube（如支持数学中的求倒数，求平方根等）。

Vector 矢量计算核，专注于矢量计算，由Scalar调度单元、矢量计算单元、Core 搬运单元等组成，不包括矩阵计算单元。
...
NPU域调 Ascend C提供的一种孪生调试方法，指基于NPU仿真软件或NPU硬件试调试。
...
算子入图算子入图指通过GE图模式运行算子，在图模式下首先将所有算子构造成一张图，然后通过GE将图下发到昇腾AI处理器执行。

算子原型算子原型是算子的抽象描述，定义了算子的输入、输出、属性等信息。

术语/缩略含义语

通算融合算通算融合算子是融合集合通信任务和计算任务的算子，在算子执行过子程中，计算和通信任务可以实现部分流水并行，从而提升性能。

逐比特模式使用Mask控制矢量计算每次Repeat内参与计算的元素时，可选择的模式之一，可以按位控制哪些元素参与计算，bit位的值为1表示参与计算，0表示不参与。

自定义算子 Ascend C提供的基于msOpGen工具生成的算子工程。工程
