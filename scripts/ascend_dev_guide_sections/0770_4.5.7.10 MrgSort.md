...
| isExhaustedS | 某条队列耗尽（即该队列已经全部排序到目的操作数）后，是否需uspension 要停止合并。类型为bool，参数取值如下：<br>- false：直到所有队列耗尽完才停止合并。<br>- true：某条队列耗尽后，停止合并。<br>默认值为false。 |
...
| sortList | 输入 | 源操作数，支持2-4个队列，并且每个队列都已经排好序，类型为MrgSortSrcList结构体，具体请参考表4-858。MrgSortSrcList中传入要合并的队列。<br>`template <typename T>`<br>`struct MrgSortSrcList {`<br>`LocalTensor<T> src1;`<br>`LocalTensor<T> src2;`<br>`LocalTensor<T> src3; // 当要合并的队列个数小于3，可以为空tensor`<br>`LocalTensor<T> src4; // 当要合并的队列个数小于4，可以为空tensor`<br>`};`<br>element 输入四个源队列的长度（排序方式一：8Bytes结构的数目，排序CountLis 方式二：16*sizeof(T)Bytes结构的数目），类型为长度为4的t uint16_t数据类型的数组，理论上每个元素取值范围[0,4095]，但不能超出UB的存储空间。 |
| sortedN | 输出 | 耗尽模式下（即isExhaustedSuspension为true时），停止合um 并时每个队列已排序的元素个数。 |
| validBit | 输入 | 有效队列个数，取值如下：<br>- 0b11：前两条队列有效<br>- 0b111：前三条队列有效<br>- 0b1111：四条队列全部有效<br>repeatTi 输入迭代次数，每一次源操作数和目的操作数跳过四个队列总长me 度。取值范围：repeatTime∈[1,255]。<br>repeatTime参数生效是有条件的，需要同时满足以下四个条件：<br>- srcLocal包含四条队列并且validBit=15<br>- 四个源队列的长度一致<br>- 四个源队列连续存储<br>- isExhaustedSuspension为false |
...
| src2 | 输入 | 源操作数，第二个已经排好序的队列。<br>类型为LocalTensor，支持的TPosition为VECIN/VECCALC/VECOUT。数据类型与目的操作数保持一致。<br>Atlas A3 训练系列产品/Atlas A3 推理系列产品，支持的数据类型为：half、float。<br>Atlas A2 训练系列产品/Atlas A2 推理系列产品，支持的数据类型为：half、float。<br>Atlas 推理系列产品AI Core，支持的数据类型为：half、float。<br>称出<br>src3 输入源操作数，第三个已经排好序的队列。<br>类型为LocalTensor，支持的TPosition为VECIN/VECCALC/VECOUT。数据类型与目的操作数保持一致。<br>Atlas A3 训练系列产品/Atlas A3 推理系列产品，支持的数据类型为：half、float。<br>Atlas A2 训练系列产品/Atlas A2 推理系列产品，支持的数据类型为：half、float。<br>Atlas 推理系列产品AI Core，支持的数据类型为：half、float。 |
