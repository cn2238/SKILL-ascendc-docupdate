...
dst 输出目的操作数，存储经过排序后的数据。

类型为LocalTensor，支持的TPosition为VECIN/VECCALC/VECOUT。

sortList 输入源操作数，支持2-4个队列，并且每个队列都已经排好序，类型为MrgSortSrcList结构体，具体请参考表4-858。MrgSortSrcList中传入要合并的队列。
...
LocalTensor<T> src3; // 当要合并的队列个数小于3，可以为空tensor LocalTensor<T> src4; // 当要合并的队列个数小于4，可以为空tensor

```cpp
};
```

element 输入四个源队列的长度（排序方式一：8Bytes结构的数目，排序CountLis 方式二：16*sizeof(T)Bytes结构的数目），类型为长度为4的t uint16_t数据类型的数组，理论上每个元素取值范围[0,4095]，但不能超出UB的存储空间。

sortedN 输出耗尽模式下（即isExhaustedSuspension为true时），停止合um 并时每个队列已排序的元素个数。
...
repeatTi 输入迭代次数，每一次源操作数和目的操作数跳过四个队列总长me 度。取值范围：repeatTime∈[1,255]。
...
src1 输入源操作数，第一个已经排好序的队列。

类型为LocalTensor，支持的TPosition为VECIN/VECCALC/VECOUT。数据类型与目的操作数保持一致。
