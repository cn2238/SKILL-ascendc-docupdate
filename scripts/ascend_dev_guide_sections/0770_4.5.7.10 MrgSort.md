...
| sortList | 输入 | 源操作数，支持2-4个队列，并且每个队列都已经排好序，类型为MrgSortSrcList结构体，具体请参考表4-858。MrgSortSrcList中传入要合并的队列。<br>`template <typename T>`<br>`struct MrgSortSrcList {`<br>`LocalTensor<T> src1;`<br>`LocalTensor<T> src2;`<br>LocalTensor<T> src3; // 当要合并的队列个数小于3，可以为空tensor LocalTensor<T> src4; // 当要合并的队列个数小于4，可以为空tensor<br>`};` |
