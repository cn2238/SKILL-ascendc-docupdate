...
| U | 源操作数数据类型。 |
| 参数名 | 描述 |
| config Fixpipe | 相关配置参数，类型为FixpipeConfig。取值如下： |
| - CFG_ROW_MAJOR（ | 默认取值）：使能NZ2ND，输出数据格式为 |
...
- 当目的操作数、源操作数 、cbufWorkspace使用基础数据类型时，
模板参数S必须为uint64_t类型，否则编译失败。

- 当目的操作数、源操作数 、cbufWorkspace使用TensorTrait类型
时，模板参数S的LiteType必须为uint64_t类型，否则编译失败。模板参数S后一个模板参数仅用于上述数据类型检查，用户无需关注。
