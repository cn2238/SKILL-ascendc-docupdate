...
| 参数名 | 描述 |
| --- | --- |
| pos TPosition | 位置，需要符合LocalMemAllocator中指定的Hardware物理位置（静态Tensor编程场景下，此参数可以省略）。 |
| DataType LocalTensor | 的数据类型，只支持基础数据类型，不支持TensorTrait类型。 |
| tileSize LocalTensor | 的元素个数，其数量不应超过当前物理位置剩余的内存空间。 |
