...
| 参数名 | 输入/输出 | 描述 |
| --- | --- | --- |
| enPartialSu | 输入 | 预留参数，用户无需感知。m<br>返回值说明<br>false：设置的SingleShape上的所有数据已经算完。<br>true：数据仍在迭代计算中。 |
...
小。每次单核计算完成后，必须将Conv3DBackpropInput对象的ctx.isFirstIter_设置为true，以确保下一轮循环中的单核计算能够正确进行。
