...
Atlas A2 训练系列产品/Atlas A2 推理系列产品，支持int8_t/int16_t/half/bfloat16_t/int32_t/float Atlas A3 训练系列产品/Atlas A3 推理系列产品，支持int8_t/int16_t/half/bfloat16_t/int32_t/float
...
- 使用完后，建议通过SetAtomicNone关闭原子最大操作，以免影响后续相关功
能。

- 对于Atlas A2 训练系列产品/Atlas A2 推理系列产品，目前无法对bfloat16_t类型
设置inf/nan模式。
...
```cpp
 ...
 extern "C" __global__ __aicore__ void data_copy_atomic_max_kernel(GM_ADDR src0_gm, GM_ADDR
...
每个核的输入数据为:Src0: [1,1,1,1,1,...,1] // 256个1 Src1: [2,2,2,2,2,...,2] // 256个2最终输出数据: [2,2,2,2,2,...,2] // 256个2
