...
scalarValue 输入激活函数中的β参数。

sharedTmpBuffer 输入临时缓存。

类型为LocalTensor，支持的TPosition为VECIN/VECCALC/VECOUT。用于SwiGLU内部复杂计算时存储中间变量，由开发者提供。

临时空间大小BufferSize的获取方式请参考4.5.3.4.2 GetSwiGLUMaxMinTmpSize。
