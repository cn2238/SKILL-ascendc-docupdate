...
| 参数名 | 输入/输出 | 描述 |
| --- | --- | --- |
| handleId | 输入 | 对应通信任务的标识ID，只能使用Prepare原语接口的返回值。<br>`using HcclHandle = int8_t;`<br>返回值说明<br>- 返回handleId对应的通信任务已执行的次数，最大值为repeat。<br>- 当执行异常时，返回-1。 |
...
HcclReduceOp::HCCL_REDUCE_SUM, repeat);
