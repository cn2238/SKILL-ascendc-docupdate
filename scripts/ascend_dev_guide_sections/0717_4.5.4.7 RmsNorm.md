...
类型为LocalTensor，支持的TPosition为VECIN/VECCALC/VECOUT。dstLocal的shape和源操作数srcLocal需要保持一致。
...
l 类型为LocalTensor，支持的TPosition为VECIN/VECCALC/VECOUT。shape需要与srcLocal和dstLocal的尾轴H长度相等，即shape为[H]。

sharedTmpB 输入临时空间。

uffer 类型为LocalTensor，支持的TPosition为VECIN/VECCALC/VECOUT。接口内部复杂计算时用于存储中间变量，由开发者提供。

临时空间大小BufferSize的获取方式请参考4.5.4.8 RmsNorm Tiling。

epsilon 输入防除零的权重系数，数据类型需要与srcLocal/dstLocal保持一致。
...
public:__aicore__ inline KernelRmsNorm()
