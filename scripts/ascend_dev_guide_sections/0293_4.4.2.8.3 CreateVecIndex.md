...
| 参数名 | 描述 |
| --- | --- |
| T | 操作数数据类型。<br>Atlas 推理系列产品AI Core，支持的数据类型为：int16_t/half/int32_t/float Atlas A2 训练系列产品/Atlas A2 推理系列产品，支持的数据类型为：int16_t/half/int32_t/float Atlas A3 训练系列产品/Atlas A3 推理系列产品，支持的数据类型为：int16_t/half/int32_t/float Atlas 200I/500 A2 推理产品，支持的数据类型为：int16_t/half/int32_t/float |
...
| repeatTime | 输入 | 重复迭代次数。矢量计算单元，每次读取连续的256Bytes数据进行计算，为完成对输入数据的处理，必须通过多次迭代（repeat）才能完成所有数据的读取与计算。<br>repeatTime表示迭代的次数。关于该参数的具体描述请参考通用参数说明。 |
| dstBlkStrid | 输入 | 单次迭代内，目的操作数不同datablock间地址步长。详e 细说明请参考dataBlockStride。 |
| dstRepStri | 输入 | 相邻迭代间，目的操作数相同datablock地址步长。详细de 说明请参考repeatStride。 |
...
结果示例如下：输入数据（firstValue）：0输出数据（dstLocal）：[0 1 2 ... 127]
