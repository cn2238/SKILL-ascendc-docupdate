...
gmWork 输入临时空间，初始化核间同步的共享内存，类型为space GlobalTensor。

ubWorks 输入临时空间，用于操作gmWorkspace，类型为LocalTensor。pace
...
gmWork 输入临时空间，通过写gmWorkspace通知其他核当前核已执行完space 成，其他核可以继续往下执行，类型为GlobalTensor。
