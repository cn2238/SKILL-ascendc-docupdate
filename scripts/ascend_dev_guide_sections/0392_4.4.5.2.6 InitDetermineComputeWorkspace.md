...
| ubWorks | 输入 | 临时空间，用于操作gmWorkspace，类型为LocalTensor。pace |
...
- gmWorkspace申请的空间最少要求为：blockNum * 32Bytes；ubWorkspace申请
的空间最少要求为：blockNum * 32 + 32Bytes；其中blockNum为调用的核数，可调用4.4.7.1 GetBlockNum获取。
