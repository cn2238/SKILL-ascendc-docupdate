...
quantPre 输入源操作数，quant tensor，量化操作时参与计算的tensor，类型为LocalTensor，支持的TPosition为C2PIPE2GM。

isUnitFla 输入 UnitFlag配置项，类型为bool。预留参数，暂未启用，为后续g 的功能扩展做保留，保持默认值false即可。

preData 输入支持设置一个Tensor，通过开关控制是relu Tensor还是quant Tensor，支持的TPosition为C2PIPE2GM。当前仅支持传入quant Tensor。
