...
表3-30 算子规格输入 Shape Data type Format
...
耗时149.04us，其中aic_mte2_ratio占比很高。设计优化方案
...
1. 步骤1 创建Matmul对象时，定义内轴非256字节对齐的B矩阵的Format为NZ格式。
```cpp
...
2. 步骤2 利用Vector计算单元实现ND2NZ格式转换。如下代码中MatrixBtoNZ为将B矩阵的ND
...
3. 步骤3 设置左矩阵A、右矩阵B、Bias，完成矩阵乘操作。
```cpp
...
| 优化方法总耗时(us) | AIC_MTE2平均耗 | AIV_MTE2平均耗时时(us) | (us) |
| --- | --- | --- | --- |
