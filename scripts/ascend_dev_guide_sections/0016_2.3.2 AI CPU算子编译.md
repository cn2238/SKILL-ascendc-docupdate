...
| --- | --- | --- |
| -help | 否 | 查看帮助。 |
| -x | 否 | 指定编译语言。<br>指定为aicpu时表示AI CPU算子编程语言。 |
...
```cpp
             ...
             CMakeLists.txt内容如下：
                ...
                # 2、指定项目支持的语言包括ASC、AICPU和CXX，ASC表示支持使用毕昇编译器对Ascend C编程语言进行编
# 译，AI CPU表示支持使用毕昇编译器对AI CPU算子进行编译
...
# set_source_files_properties进行设置
