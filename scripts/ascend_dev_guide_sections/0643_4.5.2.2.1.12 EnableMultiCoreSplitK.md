...
| 参数名 | 输入/输出 | 描述 |
| --- | --- | --- |
| flag | 输入 | 是否使能切K轴。<br>- true：使能切K轴<br>- false：不使能切K轴<br>返回值说明<br>无<br>约束说明<br>- 在算子中使用该接口时，获取C矩阵结果时仅支持输出到Global Memory。<br>- 在算子中使用该接口时，需在Kernel侧代码中首次将C矩阵分片的结果写入Global<br>Memory之前，先清零Global Memory，随后在获取C矩阵分片的结果时，再开启AtomicAdd累加。如果不预先清零Global Memory，可能会因为累加Global Memory中原始的无效数据而产生精度问题。<br>- 在算子中使用该接口时，不支持Bias参与矩阵乘计算。 |
