...
| 参数名 | 输入/输出 | 描述 |
| --- | --- | --- |
| dim | 输出 | 获取计算时所需要的核数， dim = mDim * nDim<br>mDim 输出 获取计算时M方向所需要的核数<br>nDim 输出 获取计算时N方向所需要的核数<br>batchCore 输出获取计算时batch M方向所需要的核数，仅M BatchMatmulTiling类支持<br>batchCore 输出获取计算时batch N方向所需要的核数，仅N BatchMatmulTiling类支持<br>返回值说明<br>- 1表示获取失败； 0表示获取成功。 |
