...
| enSequen | 输入输出是否连续存放数据。<br>tialWrite<br>matrixStri 输入 A矩阵源操作数相邻nd矩阵起始地址间的偏移，默认值是0。deA<br>matrixStri 输入 B矩阵源操作数相邻nd矩阵起始地址间的偏移，默认值是0。deB<br>matrixStri 输入该参数预留，开发者无需关注。deC |
...
- 单BMM内计算遵循之前的约束条件。
- 对于BSNGD、SBNGD、BNGS1S2 Layout格式，输入A、B矩阵多Batch数据总和
应小于L1 Buffer的大小。
