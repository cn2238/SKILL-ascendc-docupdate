...
| 参数名 | 输入/输出 | 描述 |
| --- | --- | --- |
| debugMo | 输入 | 表示选择的调测模式，uint8_t类型。参数支持的取值如下：de ●1：关闭HCCL高阶API通信功能。<br>- 2：打印消息队列和Prepare消息执行次数等信息。<br>- 3：打印Prepare消息源数据buffer和目的数据buffer中的<br>数据。<br>- 4：打印AI CPU服务端执行通信任务的各阶段时间戳和耗<br>时，每执行30个算子打印一次。 |
